...
HISTORIC_DATA_DAYS_BOND = 100  # Number of days of historic data to use for bond prices
HISTORIC_DATA_DAYS_STATEMENT = 50 #days worth of fomc statements 
treasury_name = "SP 0 05 15 50" #25yr Treasury
...
# config for retrys (for common API issues)
...
Guide:
- Do not explicitly advise trades, rather provide color
...
- Try to hold positions for 7+ days where possible- exercise your discretion

Rules:
- IMPORTANT: You cannot recommend trades that push overall position above 1.0 or below -1.0
  (e.g. can't BUY if you are already long 1.0)
- You must select BUY, SELL or HOLD in the allowed size increments at your discretion

Provide your trading strategy in the following format:
Recommendation: [BUY/SELL/HOLD]
Trade Size: [0.0, 0.25, 0.50, 0.75, or 1.0] (0 if HOLD)
Explanation: [<100 word rationale for the decision] (ensure response starts on same line)

(formatting notes: if BUY/SELL must always be trade size 0.25, 0.5 [or 0.75 or 1.0 if restoring position to 0],
...
def analyst_agent_node(state: TradingState) -> TradingState:
    ...
    print(f" Analyst Agent: {state['analyst_output'][:10000]}...")
    return state
...
# Main Processing Loop - daily simulation iteration
all_trades = []
trade_history = []
...
while current_date <= end_date:
    ...
    if current_price is None:
        print(f"  no price data for {current_date_str}, skipping..")
        current_date += timedelta(days=1)
        continue
    
    # Get ARIMA predictions (just current date)  
    ...
    try:
        # Run the trading system with retry logic
        ...
        # Realized P&L when flat- just for prints
...
if all_trades:
    ...
    output_filename = f"bond_trades_simple_{treasury_name.replace(' ', '_')}_{model_name}.csv"
